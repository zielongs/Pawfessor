...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$title || !$category || !$priority || !$due_date || !$due_time) {
        ...
        $sql_update = "UPDATE tasks 
               SET title=?, subject=?, category=?, due_date=?, due_time=?, priority=?, notes=? 
               WHERE id=? AND user_id=?";
$stmt_update = mysqli_prepare($conn, $sql_update);
mysqli_stmt_bind_param(
    $stmt_update,
    "sssssssii", // 7 strings + 2 integers (task_id, user_id)
    $title,
    $subject,
    $category,
    $due_date,
...
<div class="container">
    ...
    <input 
        type="time" 
        name="taskDueTime" 
        value="<?php echo isset($task['due_time']) ? substr($task['due_time'], 0, 5) : ''; ?>"
        required
    >
</div>
